import useSortByData from "../../../Hooks/useSortByData";

const TopSeller = () => {

    const data = useSortByData()
    console.log(data)
            ...
            <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-4 w-10/12 mx-auto">

                {
                    data?.slice(0,3)?.map(item => (
                        <div key={item._id} className="flex flex-col bg-[#fffcf7] justify-center max-w-xs p-4 shadow-md rounded-xl sm:px-12 dark:bg-gray-900 dark:text-gray-100">
                            <img src={item.image} alt="" className="w-32 h-32 mx-auto rounded-full dark:bg-gray-500 aspect-square" />
                            <div className="space-y-4 text-center divide-y dark:divide-gray-700">
                                <div className="my-2 space-y-1">
                                    <h2 className="text-xl font-semibold sm:text-2xl">{item?.userName}</h2>
                                    <p className="px-5 text-xs sm:text-base dark:text-gray-400">Total Delivered: {item?.totalDeliver}</p>
                                </div>
